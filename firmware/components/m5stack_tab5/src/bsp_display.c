 ...
 */
/*
 * TOUCH DECK PATCH: upstream only probes the GT911 backup address (0x14) and
 * hard-asserts when the controller responds on its primary address (0x5D)
 * instead - which is what this unit does. Track whichever address answered so
 * bsp_touch_new() below can use the same one instead of hardcoding backup.
 */
static uint8_t s_gt911_addr = ESP_LCD_TOUCH_IO_I2C_GT911_ADDRESS_BACKUP;
...
static bsp_tab5_board_version_t bsp_get_board_version(void)
{
    ...
    if (i2c_master_probe(bsp_i2c_get_handle(), ESP_LCD_TOUCH_IO_I2C_ST7123_ADDRESS, 100) == ESP_OK) {
        ...
        ESP_LOGI(TAG, "Discovered board version 1 (LCD ILI9881C, Touch GT911 @ 0x%02x)",
                 ESP_LCD_TOUCH_IO_I2C_GT911_ADDRESS_BACKUP);
        s_gt911_addr = ESP_LCD_TOUCH_IO_I2C_GT911_ADDRESS_BACKUP;
        board_ver = BSP_TAB5_BOARD_VERSION_ILI9881C_GT911;
    } else if (i2c_master_probe(bsp_i2c_get_handle(), ESP_LCD_TOUCH_IO_I2C_GT911_ADDRESS, 100) == ESP_OK) {
        ESP_LOGI(TAG, "Discovered board version 1 (LCD ILI9881C, Touch GT911 @ 0x%02x)",
                 ESP_LCD_TOUCH_IO_I2C_GT911_ADDRESS);
        s_gt911_addr = ESP_LCD_TOUCH_IO_I2C_GT911_ADDRESS;
        board_ver = BSP_TAB5_BOARD_VERSION_ILI9881C_GT911;
    } else {
        ESP_LOGE(TAG, "Unsupported board version! (no known LCD/touch responded on I2C)");
        return BSP_TAB5_BOARD_VERSION_UNKNOWN;
    }
...
esp_err_t bsp_touch_new(const bsp_touch_config_t *config, esp_lcd_touch_handle_t *ret_touch)
{
    ...
    /* Enable Feature */
    /* The touch IC stays powered across MCU/USB resets and can retain a stale
     * interrupt state. Give it a real reset before creating the driver. */
    ...
    if (board_version == BSP_TAB5_BOARD_VERSION_ILI9881C_GT911) {
        ...
        */
        ...
        /* TOUCH DECK PATCH: use whichever address bsp_get_board_version() found,
         * not a hardcoded backup address (see s_gt911_addr above). */
